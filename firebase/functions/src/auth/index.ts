...
export async function isAuthenticated(
  ...
  try {
    ...
    // Stores the decoded token within the locals for further processing
 ...
 * @param {Object} hasRole
 * @param {Object} opts
 * @return {Object}
 */
export function isAuthorized(
    hasRole: string[],
    opts?: { allowSameUser?: boolean; allowRootEmail?: boolean }
) {
  return (req: Request, res: Response, next: Function) => {
    ...
    // Use firebase functions:config:set root.email="value" to set the variables
    if (
      opts?.allowRootEmail &&
      context.email &&
      context.email === functions.config().root?.email
    ) {
